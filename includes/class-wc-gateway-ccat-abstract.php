...
abstract class WC_Gateway_CCat_Abstract extends WC_Payment_Gateway
{
    ...
    public function process_payment($order_id): array
    {
        ...
        if (is_wp_error($response)) {
            ...
            throw new Exception(esc_html(__('Api error. try again later.', 'woocommerce-gateway-ccat')));
        }
        ...
        if (isset($response_data['status']) && self::CODE_OK === $response_data['status']) {
            ...
            $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::PENDING, __('Awaiting payment', 'woocommerce-gateway-ccat'));
            WC()->cart->empty_cart();
            ...
            $error_message = $response_data['msg'] ?? __('Unknown Error.', 'woocommerce-gateway-ccat');
    ...
    public function handle_payment_callback(WP_REST_Request $request): WP_REST_Response
    {
        ...
        switch ($status) {
            case 'B':
                $note = __('授權完成', 'woocommerce-gateway-ccat');
            ...
            case 'O':
                $note = __('請款作業中', 'woocommerce-gateway-ccat');
                $order->update_meta_data(self::META_PROCESSING, 1);
                $order->save();
                break;

            case 'E':
                $note = __('請款完成', 'woocommerce-gateway-ccat');
                $order->update_status('completed');
                break;

            case 'F':
                $note = __('授權失敗', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::FAILED);
                break;

            case 'D':
                $note = __('訂單已逾期', 'woocommerce-gateway-ccat');
                break;

            case 'P':
                $note = __('請款失敗', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::FAILED);
                break;

            case 'M':
                $note = __('取消交易完成', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::CANCELLED);
                break;

            case 'N':
                $note = __('取消交易失敗', 'woocommerce-gateway-ccat');
                break;

            case 'Q':
                $note = __('取消授權完成', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::CANCELLED);
                break;

            case 'R':
                $note = __('取消授權失敗', 'woocommerce-gateway-ccat');
                break;

            case 'I':
                $note = __('開立發票通知', 'woocommerce-gateway-ccat');
                break;

            case 'J':
                $note = __('開立發票折讓通知', 'woocommerce-gateway-ccat');
                break;

            default:
                $note = __('未知狀態碼: ', 'woocommerce-gateway-ccat') . $status;
                break;
        }
        ...
        if (isset($data['invoice_no'])) {
            ...
            $note = sprintf(
                __('已收到電子發票通知，發票號碼：%s，開立日期：%s，隨機碼：%s', 'woocommerce'),
                $data['invoice_no'],
                $data['invoice_date'],
                $data['random_number']
            );
