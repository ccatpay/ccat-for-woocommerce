...
abstract class WC_Gateway_CCat_Cvs_Abstract extends WC_Gateway_CCat_Abstract
{
    ...
    public function process_payment($order_id): array
    {
        ...
        try {
            ...
            if (empty($payer_name)) {
                throw new Exception(esc_html__('姓名為必填', 'woocommerce-gateway-ccat'));
            }
            if (empty($payer_postcode)) {
                throw new Exception(esc_html__('郵遞區號為必填', 'woocommerce-gateway-ccat'));
            }
            if (empty($payer_address)) {
                throw new Exception(esc_html__('地址為必填', 'woocommerce-gateway-ccat'));
            }
            if (empty($payer_mobile)) {
                throw new Exception(esc_html__('手機為必填', 'woocommerce-gateway-ccat'));
            }
            ...
            if (isset($response_data['status']) && self::CODE_OK === $response_data['status']) {
                ...
                $order->update_status(
                    ...
                    __('Awaiting payment', 'woocommerce-gateway-ccat')
                );
                $order->save();
                WC()->cart->empty_cart();
    ...
    public function handle_payment_callback(WP_REST_Request $request): WP_REST_Response
    {
        ...
        switch ($status) {
            case 'A':
                $note = __('等待繳款人繳納', 'woocommerce-gateway-ccat');
                $order->update_status('pending');
                break;

            case 'B':
                $note = __('繳款人已繳納', 'woocommerce-gateway-ccat');
                $order->set_payment_method($this);
                $order->payment_complete();
                break;

            case 'C':
                $note = __('契客註定註銷', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::CANCELLED);
                break;

            case 'D':
                $note = __('已過期繳款單', 'woocommerce-gateway-ccat');
                $order->update_status(Automattic\WooCommerce\Enums\OrderStatus::CANCELLED);
                break;

            case 'E':
                $note = __('已預約撥款給契客', 'woocommerce-gateway-ccat');
                break;
            ...
            case 'J':
                $note = __('開立發票折讓單號通知', 'woocommerce-gateway-ccat');
                break;
